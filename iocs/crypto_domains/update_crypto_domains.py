...
class CryptoDomains:
    ...
    def update_crypto_domains(self):
        if self.url_crypto_domains:
            ...
            print(f"[{time.strftime('%H:%M:%S')}] [ERROR] URL for crypto / cryptojacking based sites list is not present in the config ...")
            logging.error(f"URL for crypto / cryptojacking based sites list is not present in the config")
            print("\nExiting program ...\n")
            sys.exit(1)